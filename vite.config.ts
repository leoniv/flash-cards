...
export default defineConfig({
  plugins: [react(), FullReload(["static/cards/**/*"]), ManifestPlugin()],
  publicDir: "static",
  build: {
    outDir: "public",
    emptyOutDir: true
...
function ManifestPlugin(): PluginOption {
  async function generate() {
    ...
    // Log discovered markdown files (optional)
    files.forEach((f) => console.log(`[cards] ${f}`));
    const cards = files
    ...
    console.log(`[cards] Generated manifest with ${cards.length} cards`);
  }
  return {
    name: "cards-manifest",
    async buildStart() { await generate() },
    configureServer(server) {
      const run = () => generate().then(() =>
